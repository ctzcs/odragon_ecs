...
|---|---|---|---|
| Create 1M entities | **61 ms** (25.7 reserved) | 141 ms | 235 ms |
| Iterate (Pos) ×1M | 1.20 ms | **0.41 ms** | 0.40 ms |
| Iterate (Pos+Vel, write) ×1M | 1.86 ms | **1.20 ms** | 0.96 ms |
| Iterate (Pos+Vel+Health) ×1M | 2.02 ms | **1.49 ms** | 1.23 ms |
| Sparse iteration ×100k | 0.50 ms | **0.04 ms** | 0.04 ms |
| Query Pos+Vel exc Tag ×900k (cached, real pass) | **0.06 ms** | 0.15 ms | 0.20 ms |
| Random access ×1M (permutation) | **3.55 ms** | 22.1 ms | 34.1 ms |
| Churn: add+remove ×100k | **0.67 ms** | 7.4 ms | 11.7 ms |
| Delete 1M entities | 44.3 ms | **16.1 ms** | 66.5 ms |

**Reading it**: flecs packs components per archetype, so bulk iteration is ~1.4–2.7x faster and sparse iteration ~12x faster; ODragonECS's sparse sets win everything structural (create 2.3–5.4x, churn 10–17x, random access 6–10x) plus cached-query iteration (2.4–3.4x). Choose by workload: iteration-dominated (rendering, physics sweeps) favors archetypes; churn-heavy gameplay (spawning, buffs, events) favors sparse sets. (flecs delete spread — 14.5 ms via LLVM-compiled binding vs 66.5 ms via GCC — is compiler codegen variance in flecs internals, not measurement error.)
...
|---|---|---|---|
| 创建 100 万实体 | **61 ms**(预留 25.7) | 141 ms | 235 ms |
| 迭代 (Pos) ×100 万 | 1.20 ms | **0.41 ms** | 0.40 ms |
| 迭代 (Pos+Vel,写) ×100 万 | 1.86 ms | **1.20 ms** | 0.96 ms |
| 迭代 (Pos+Vel+Health) ×100 万 | 2.02 ms | **1.49 ms** | 1.23 ms |
| 稀疏迭代 ×10 万 | 0.50 ms | **0.04 ms** | 0.04 ms |
| 查询 Pos+Vel exc Tag ×90 万(缓存后真实遍历) | **0.06 ms** | 0.15 ms | 0.20 ms |
| 随机访问 ×100 万(全排列打乱) | **3.55 ms** | 22.1 ms | 34.1 ms |
| churn:增删 ×10 万 | **0.67 ms** | 7.4 ms | 11.7 ms |
| 删除 100 万实体 | 44.3 ms | **16.1 ms** | 66.5 ms |

**解读**:flecs 按 archetype 紧凑打包组件,批量迭代快 1.4~2.7 倍、稀疏迭代快 12 倍;ODragonECS 的稀疏集在所有结构性操作上大幅领先(创建 2.3~5.4 倍、churn 10~17 倍、随机访问 6~10 倍),缓存查询遍历也快 2.4~3.4 倍。按负载选型:迭代主导(渲染、物理扫描)选 archetype;churn 主导(刷怪、buff、事件)选稀疏集。(删除一项 flecs 两个构建差 4.6 倍——LLVM 编译的绑定 16.1ms vs GCC 66.5ms——是 flecs 内部代码的编译器差异,不是测量误差。)
